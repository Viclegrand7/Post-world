...
void Physic :: Ennemy :: testCollisionWall(const std :: vector<CollisionBox *> &walls, unsigned int curFrame) {
	Vector3f tmp(att_fakeBox[curFrame].att_center);
	for (unsigned int i = 0 ; i < walls.size() ; ++i)
		Physic :: Collision :: sphereBox(att_fakeBox[curFrame], *walls[i]);
...
void Physic :: Ennemy :: update(const Vector3f &character, const std :: vector<Ennemy *> &ennemies, const std :: vector <CollisionBox *> &wall, float gravity, unsigned int curFrame) {
