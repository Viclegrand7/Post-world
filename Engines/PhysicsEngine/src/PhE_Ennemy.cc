...
void Physic :: Ennemy :: testCollisionWall(const std :: vector<CollisionBox *> &walls) {
	...
	for (unsigned int i = 0 ; i < walls.size() ; ++i)
		Physic :: Collision :: sphereBox(att_fakeBox, *walls[i]);
...
bool Physic :: Ennemy :: doesGetHit(const Vector3f &pos, const Vector3f &dir, float *dis, Vector3f *collisionPoint) {
	...
	bool tmp(	Physic :: Collision :: raySphere(pos, dir, att_head, dis, collisionPoint)); 	//Headshot;
		...
		tmp &=	Physic :: Collision :: rayBox(pos, dir, att_arms[0], dis, collisionPoint);		//Arm shot
		tmp &=	Physic :: Collision :: rayBox(pos, dir, att_arms[1], dis, collisionPoint);		//Arm shot
		tmp &=	Physic :: Collision :: rayBox(pos, dir, att_legs[0], dis, collisionPoint);		//Leg shot
		tmp &=	Physic :: Collision :: rayBox(pos, dir, att_legs[1], dis, collisionPoint);		//Leg shot
	return tmp;
}

void Physic :: Ennemy :: update(const Vector3f &character, const std :: vector<Ennemy *> &ennemies, const std :: vector <CollisionBox *> &wall, float gravity) {
