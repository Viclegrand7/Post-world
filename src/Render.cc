...
inline void Render :: clear() {
	SDL_RenderClear(renderer); //Currently hating this, better call the function everytime
}

inline void Render :: render(SDL_Texture *texture) {
	SDL_Rect source;
	source.x = 0; source.y = 0; //Starting at top left of the image 
	source.width = ; source.height = ; //Using some size

	SDL_Rect destination;
	destination.x = 0; destination.y = 0; //Starts top left
	destination.width = ; destination.height = ; //Where it ends

	SDL_Render_Copy(renderer, texture, &source, &destination); //Last 2 pointers: How gig source is, destination (transformations)
}

inline void Render :: display() {
	SDL_RenderPresent(renderer); //Same
}